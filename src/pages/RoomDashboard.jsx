    ...
    try {
      ...
      if (existingInvite) {
        ...
        if (existingInvite.status === 'pending') {
          ...
          if (memberCheck) {
            inviteState.setError(`${inviteEmail.trim()} is already a member of this room!`)
            setError(`${inviteEmail.trim()} is already a member of this room!`)
            return
          }
        } else {
          // Rejected or other status - resend by updating
          const { error: updateError } = await supabase
            .from('room_invites')
            .update({
              status: 'pending',
